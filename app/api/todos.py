...
from db.database import get_db_shard, get_all_dbs
...
def read_todos(dbs: Session = Depends(get_all_dbs)):
    db1, db2 = dbs
    return crud_todos.get_todos(db1) + crud_todos.get_todos(db2)


@router.get("/todos/{todo_id}", response_model=TodoItem)
def read_todo(todo_id: int, db: Session = Depends(get_db_shard)):
...
def create_todo(todo: TodoCreate, db: Session = Depends(get_db_shard)):
    return crud_todos.create_todo(db, todo)


@router.put("/todos/{todo_id}", response_model=TodoItem)
def update_todo(todo_id: int, todo: TodoCreate, db: Session = Depends(get_db_shard)):
...
def delete_todo(todo_id: int, db: Session = Depends(get_db_shard)):
    db_todo = crud_todos.get_todo_by_id(db, todo_id)
    if db_todo is None:
        raise HTTPException(status_code=404, detail="Task not found")
